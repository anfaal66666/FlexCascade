#!/usr/bin/env python3
"""Quick CourtListener - just read clusters until N complete rows found."""
...
def main():
    ...
    with gzip.open(output_path, "wt", encoding="utf-8", newline="") as out_f:
        writer = csv.DictWriter(out_f, fieldnames=["id", "title", "citation", "docket_number", "state", "issuer", "document", "timestamp"])
        writer.writeheader()
            ...
            for i, row in enumerate(csv.DictReader(f)):
                try:
                    if i % 1000000 == 0 and i > 0:
                        print(f"  Checked {i:,}, written {rows_written:,}...", flush=True)

                    cluster_id = s(row.get("id"))
                    docket_id = s(row.get("docket_id"))

                    if not cluster_id or not docket_id or cluster_id not in opinions_map:
                        continue
                    ...
                    if rows_written >= args.rows:
                        print(f"  Reached {args.rows:,} rows, stopping\n")
                        break
                except:
                    pass
