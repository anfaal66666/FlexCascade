#!/usr/bin/env python3
"""Fast CourtListener sampler - read first 30M of each file, extract N matching rows."""
...
def parse_args():
    ...
    parser.add_argument("--sample-limit", type=int, default=30000000)
...
def main():
    ...
    print(f"Pass 1: Reading first {args.sample_limit:,} clusters...")
        ...
        for row in reader:
            try:
                ...
                if rows_seen % 1000000 == 0:
                    print(f"  Read {rows_seen:,}...", flush=True)
                    sys.stdout.flush()

                if rows_seen > args.sample_limit:
                    break
    ...
    print(f"  Collected {len(clusters_data):,} clusters, need {len(needed_docket_ids):,} dockets\n")

    print(f"Pass 2: Reading first {args.sample_limit:,} dockets...")
    dockets_map = {}
    rows_seen = 0
    ...
    with bz2.open(dockets_path, "rt", encoding="utf-8", errors="ignore") as f:
        ...
        for row in reader:
            try:
                rows_seen += 1
                if rows_seen % 1000000 == 0:
                    print(f"  Scanned {rows_seen:,}, kept {len(dockets_map):,}...", flush=True)
                    sys.stdout.flush()
    ...
    print(f"Pass 3: Reading opinions until we have {args.rows:,} matches...")
    ...
    with bz2.open(opinions_path, "rt", encoding="utf-8", errors="ignore") as f:
        ...
        for row in reader:
            try:
                rows_seen += 1
                if rows_seen % 5000000 == 0:
                    print(f"  Scanned {rows_seen:,}, found {len(opinions_map):,}...", flush=True)
                    sys.stdout.flush()
                ...
                if cluster_id in needed_cluster_ids and cluster_id not in opinions_map:
                    ...
                    if text:
                        ...
                        if rows_found >= args.rows:
                            print(f"  Found {rows_found:,} matching opinions, stopping\n")
                            break
            except:
                pass

    print(f"Pass 4: Writing {args.rows:,} rows...")
    rows_written = 0
